    ...
    success: function (data) {
      for  (var i = 0; i < data.length; i++) {
        if (repos.indexOf(data[i].full_name) !== -1) {
          x = data[i].name;
          $("div[repo='" + x + "']").children(".star").html('<i class="far fa-star"></i> ' + data[i].stargazers_count)
          $("div[repo='" + x + "']").children(".fork").html('<i class="fas fa-code-branch"></i> ' + data[i].forks_count)
        }
      }
    }
  })}
